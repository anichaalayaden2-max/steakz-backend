...
export const getHQStats = async (req, res) => {
    try {
        ...
        const branchStats = branches.map((branch) => {
            const revenue = branch.Order.reduce((sum, order) => sum + order.Payment.reduce((paymentSum, payment) => paymentSum + payment.amount, 0), 0);
            return {
                id: branch.id,
                name: branch.name,
